package com.example.shardacarpooling.Single_Day.Ghaziabad;
...
public class gh_book extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gh_book2);
        ...
        sysID = findViewById(R.id.s_gh_sysID3);
        name = findViewById(R.id.s_gh_name2);
        seats = findViewById(R.id.s_gh_total_seats);
        ...
        book.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(PASSid.length() != 10 ){
                    Toast.makeText(gh_book.this, "Invalid System ID", Toast.LENGTH_SHORT).show();
                    return;
                }

                if(number.length()!=10){
                    Toast.makeText(gh_book.this, "Invalid Mobile Number", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if(!PASSid.equals(dr_sys)){
                    ...
                    databaseReference.child("Ghaziabad").child(drID).child("Total_Seats").setValue(getSeats);
                    databaseReference.child("Ghaziabad").child(drID).child("Passengers").child(PASSid).child("Passenger_Name").setValue(name);
                    databaseReference.child("Ghaziabad").child(drID).child("Passengers").child(PASSid).child("Passenger_ID").setValue(PASSid);
                    databaseReference.child("Ghaziabad").child(drID).child("Passengers").child(PASSid).child("Passenger_Number").setValue(number);

                    Toast.makeText(gh_book.this, "Seat Booked", Toast.LENGTH_SHORT).show();

                    startActivity(new Intent(gh_book.this, Seat_Booked_Successfully.class));
                }

                else{
                    Toast.makeText(gh_book.this, "Driver and Passenger ID cannot be Same", Toast.LENGTH_SHORT).show();
                    return;
                }
            }
        });
